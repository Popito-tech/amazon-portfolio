...
const CartPayment = () => {
    const { cartData, userInfo } = useSelector(
      ...
      useEffect(() => {
        ...
        cartData.map((item:StoreProduct) => {
            priceStaker += item.price * item.quantity;
          return;
        });
        setTotalAmount(priceStaker);
      }, [cartData]);
        ...
        const handleCheckout = async () => {
          ...
          const response = await fetch("/api/checkout", {
            ...
            body: JSON.stringify({ items: cartData, email: session?.user?.email }),
          });
      ...
      {userInfo ? (
        <div className="flex flex-col items-center">
          ...
          onClick={handleCheckout}
            className="w-full h-10 text-sm font-semibold button"
          >
            Proceed to Buy
          </button>
        </div>
